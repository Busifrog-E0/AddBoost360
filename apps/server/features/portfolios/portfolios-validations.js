...
import { PortfolioSchema } from "validations"


/**
 * Validate POST /portfolio
 */
const ValidatePostPortfolio = (req, res, next) => {
  const { error, value } = PortfolioSchema.validate(req.body, { stripUnknown: true });
...
/**
 * Validate GET /portfolio
 */
const ValidateGetPortfolio = (req, res, next) => {
...
/**
 * Validate PATCH /portfolio
 */
const ValidatePatchPortfolio = (req, res, next) => {
  const { error, value } = PortfolioSchema.validate(req.body, { stripUnknown: true });
  if (error) return void res.status(400).json(error.details);
  req.body = value;
  return next();
